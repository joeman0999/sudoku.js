...
(function(root){
    ...
    sudoku.generate = function(difficulty, unique){
        ...
        */
        ...
        if(typeof difficulty === "string" || typeof difficulty === "undefined"){
            ...
            var min = DIFFICULTY_RANGES[difficulty]["min"];
        ...
        // Default unique to true if unspecified
        if (unique !== false) {
            unique = true;
        }
    ...
    sudoku.grid_to_board_map = function(old_gird) {
        /* Convert a board grid to a map
        */
        var new_board = {};
        for(var r = 0; r < 9; ++r){
            for(var c = 0; c < 9; ++c){
                new_board[SQUARES[r*9 + c]] = old_gird[r][c];
            }   
        }
        return new_board;
    }

    sudoku.board_map_to_grid = function(old_board) {
        /* Convert a board map to a grid
        */
        var new_grid = [];
        for(var r = 0; r < 9; ++r){
            new_grid.push([]);
            for(var c = 0; c < 9; ++c){
                new_grid[r].push(old_board[SQUARES[r*9 + c]]);
            }   
        }
        return new_grid;
    }
